<template>
    <div class="input-error-group">
        ...
        <select v-model="internalValue" v-bind="$attrs" @blur="touched = true">
            <option value="" disabled>Chọn thế hệ</option>
            <option v-for="(item, index) in options" :value="item[fieldKey]">Thế hệ {{item[fieldData]}}</option>
        </select>
...
<script setup>
    ...
    const fieldKey = 'idGeneration'
    const fieldData = 'idGeneration'
