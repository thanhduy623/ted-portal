<template>
    <div class="input-error-group">
        <select v-model="internalValue">
            <option v-for="(opt, index) in options" :key="index" :value="opt.idSchoolYear">
                Năm học {{ opt.nameSchoolYear }}
            </option>
        </select>
    </div>
</template>
...
<script setup>
    ...
    const options = sessionGet("masterData")?.schoolYearList.reverse() || []

    // ✅ Gán giá trị mặc định nếu chưa có
    const internalValue = ref(props.modelField || options[0]?.idSchoolYear || '')
